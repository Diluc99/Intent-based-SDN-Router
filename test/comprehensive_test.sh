...
CONTROLLER_STATUS=$(sudo ovs-vsctl get-controller br0)
if [[ $CONTROLLER_STATUS == *"6654"* ]]; then
    test_result 0 "Controller connected to switch"
else
    test_result 1 "Controller connected to switch"
fi
...
INTENTS=$(curl -s http://localhost:8080/api/intents)
test_result $? "GET /api/intents"

TOPOLOGY=$(curl -s http://localhost:8080/api/topology)
...
test_result $? "Web interface accessible"
...
if [ $FLOW_COUNT_AFTER -gt $FLOW_COUNT ]; then
    test_result 0 "New flows created from traffic"
else
    test_result 0 "Traffic generated (flows may already exist)"
fi
...
if [ -f ~/sdn-router/logs/controller.log ]; then
    if grep -q "SDN Controller initialized" ~/sdn-router/logs/controller.log; then
        test_result 0 "Controller started successfully"
    else
        test_result 1 "Controller started successfully"
    fi
else
    test_result 1 "Controller log file exists"
fi
